...
class Authenticate:
    ...
    def _check_account(self, inplace: bool = True) -> bool:
        account_info = AccountModel.get_account(username=self.username)
        if account_info:
            try:
                account_info = account_info[0]
                if bcrypt.checkpw(self.password.encode(), account_info['password'].encode()):
                    if inplace:
                        st.session_state['name'] = account_info['username']
    ...
    def _register_credentials(self, username: str, invitation_code: str, password: str, email: str, preauthorization: bool):
        """
        ...
        invitation_code: str
            The invitation code of the new user.
        password: str
        ...
        AccountModel.add_account(email=email,
                                 username=username,
    ...
    def register_user(self, form_name: str, location: str = 'main', preauthorization=True) -> bool:
        ...
        new_invitation_code = register_user_form.text_input('Invitation Code')
        new_password = register_user_form.text_input('Password', type='password')
        new_password_repeat = register_user_form.text_input('Repeat password', type='password')

        if register_user_form.form_submit_button('Register'):
            if not (len(new_email) and len(new_username) and len(new_invitation_code) and len(new_password) > 0):
                raise RegisterError('Please enter an email, username, name, and password')
            # TODO: you need modify this logics
            if not (str(new_invitation_code) in ['daoootop666']):
                raise RegisterError('Please use new Invitation Code')

            if AccountModel.get_account(new_username):
                raise RegisterError('username exist, Please use new one')
            ...
            if preauthorization:
                if new_email in self.preauthorized['emails']:
                    self._register_credentials(new_username, new_invitation_code, new_password, new_email,
                                               preauthorization)
                    return True
                else:
